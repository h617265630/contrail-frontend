...
export function getMyProgressForItem(pathItemId: number) {
  return request.get<ProgressRow, ProgressRow>(`/progress/me/item/${pathItemId}`)
}

export function upsertMyProgress(payload: { path_item_id: number; progress_percentage: number }) {
