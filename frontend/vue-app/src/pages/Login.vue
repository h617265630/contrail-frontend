...
function validateForm() {
  ...
  if (!email.value) newErrors.email = 'Username is required'
  else if (email.value.trim().length < 2) newErrors.email = 'Username is too short'
...
async function handleSubmit() {
  ...
  try {
    ...
    formError.value = e?.detail || e?.message || '登录失败，请稍后再试'
  } finally {
    loading.value = false
  }
}
